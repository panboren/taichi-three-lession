    ...
    <div class="ui-layer">
      ...
      <div class="narrative-container">
        <div ref="textRef" class="narrative-text">
          <h1 class="glitch-text">{{ currentSceneData.title }}</h1>
          <p class="subtitle">{{ currentSceneData.subtitle }}</p>
        </div>
      </div>

      <div class="scroll-hint" :class="{ hidden: isScrolling }">
        <span>{{ t('digitalSpace.hints.scroll') }}</span>
      ...
      <div class="interaction-hint">
        <span class="hint-text">{{ t('digitalSpace.hints.interaction') }}</span>
      </div>
    </div>
...
const currentSceneData = computed(() => {
  const sceneKey = String(currentScene.value) as '1' | '2' | '3'
  return t(`digitalSpace.scenes.${sceneKey}`) as {
    title: string
    subtitle: string
    description: string
  }
